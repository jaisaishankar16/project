...
def home():
    ...
    if form.validate_on_submit():
        ...
        existing_file = images_collection.find_one({'filename': filename})

# If the filename does not exist, insert the document
        ...
        if not existing_file:
            ...
            print("Error: File with the same name already exists.Please give a unique name")
...
def dashboard():
    ...
    total_documents = images_collection.count_documents({'username': username})

    return render_template('dashboard.html', user_images=user_images, username=username, total_documents=total_documents)
...
# Define route for deleting a document
@app.route('/delete', methods=['POST'])
def delete_image():
    # Get the _id and filename of the document from the form
    ...
    # Do whatever you need with the filename here
    
    return redirect('/dashboard ')  # Redirect to the main page after deletion


@app.route('/sum_total', methods=['GET'])
def sum_total():
    total_sum = images_collection.aggregate([
        {"$group": {"_id": None, "total": {"$sum": "$total"}}}
    ])

    total = list(total_sum)[0]['total'] if total_sum else 0
